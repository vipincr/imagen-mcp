...
from .core import (
    API_KEY_ENV,
    ImageResult,
    edit_image as core_edit_image,
    generate_image as core_generate_image,
    generate_image_resized,
    generate_image_with_references as core_generate_with_refs,
...
def generate_image(
    ...
    Args:
        ...
        output_path: Optional path to save the image. If provided, saves to file instead of
                    returning base64 data. Include the file extension (e.g., "/path/to/image.png").
        ...
        max_width: Optional maximum width for resizing. Must be used with max_height.
        max_height: Optional maximum height for resizing. Must be used with max_width.
        format: Optional output format when resizing (png, jpeg, webp).
        quality: Optional quality for JPEG/WebP compression (1-100, default 85).

    Returns:
        A dictionary containing:
        - success: Boolean indicating if generation succeeded
        - image_base64: Base64-encoded image data (if no output_path provided)
        - saved_path: Absolute path where the image was saved (if output_path provided)
        ...
        - resized: Boolean indicating if the image was resized (only if max_width/max_height provided)
        - error: Error message (if failed)
    """
    def _run():
        extra = {}
        should_resize = max_width is not None and max_height is not None

        if should_resize:
            result = generate_image_resized(
                prompt=prompt,
                max_width=max_width,
            ...
            extra = {"resized": True, "max_width": max_width, "max_height": max_height}
        else:
            result = _call_with_aspect_ratio_fallback(
                core_generate_image,
                aspect_ratio=aspect_ratio,
                prompt=prompt,
                model_id=model,
        ...
        return _handle_image_result(result, model=model, output_path=output_path, extra=extra if extra else None)
...
def edit_image(
    ...
    Args:
        ...
        output_path: Optional file path to save the edited image.
                    If not provided, returns base64-encoded data.
    ...
    Returns:
        ...
        - image_base64: Base64-encoded edited image data (if no output_path provided)
        - saved_path: Absolute path where the image was saved (if output_path provided)
        - mime_type: MIME type of the image (e.g., "image/png")
        - extension: Suggested file extension (e.g., ".png")
        - size_bytes: Size of the image in bytes
...
def generate_image_with_references(
    ...
    Args:
        reference_paths: List of 1-3 file paths to reference images.
        prompt: A detailed description of what to generate and how to use the references.
        output_path: Optional path to save the image. If provided, saves to file.
        aspect_ratio: Optional aspect ratio for the generated image.
    ...
    Returns:
        A dictionary with image data or saved path, mime_type, size_bytes, and reference_count.
    """
    def _run():
        ...
        if should_resize:
            ...
            extra.update({"resized": True, "max_width": max_width, "max_height": max_height})
        else:
            result = _call_with_aspect_ratio_fallback(
                core_generate_with_refs,
                aspect_ratio=aspect_ratio,
                prompt=prompt,
                reference_images=refs,
                model_id=model,
        ...
        return _handle_image_result(result, model=model, output_path=output_path, extra=extra)
...
def save_image(
    ...
    This tool saves image data (typically from generate_image) to the filesystem.
    It will create any necessary parent directories.

    Args:
        image_base64: Base64-encoded image data from generate_image.
...
def convert_image(
    ...
    Args:
        ...
        format: Target format (png, jpeg, jpg, webp, heic, heif, ico).
        sizes: Optional list of sizes for ICO files (e.g., [16, 32, 48, 64, 128]).

    Returns:
        A dictionary containing:
        - success: Boolean indicating if conversion succeeded
        - saved_path: Absolute path where the image was saved
        - mime_type: MIME type of the output
        - format: The format used
        - sizes: The sizes used (for ICO)
        - error: Error message (if failed)
    """
    try:
